...
import se.uu.farmbio.cp.alg.LIBLINEAR._
...
class LIBLinearTest extends FunSuite with SharedSparkContext{
  ...
  test("util function takeAbsolute"){
    ...
    assert(lists.length==3, 
        ...
        "(1 more than the specified numbers to take, last index containing all remaining elements)");
  ...
  test(" save/load LibLinAlg"){
    ...
    val params = new LibLinParams();
    val Array(test, calib, propTraining) = LIBLINEAR.takeFractions(largeTestData.toList, Array(0.2,0.2,.6))
    ...
    test.foreach{ lp =>
      assert(  model.predict(lp.features, significance).equals(model2.predict(lp.features, significance)) && 
               model.predict(lp.features, significance).equals(model3.predict(lp.features, significance)), 
